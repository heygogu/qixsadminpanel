...
const StaffFormSkeleton = () => {
  return (
    <div className="animate-in fade-in-50 grid grid-cols-2 duration-500 slide-in-from-bottom-5">
...
const CreateStaff = () => {
  ...
  const [updatedUserTabs, setUpdatedUserTabs] = useState<string[]>([]);
  const TabsList = [
  ...
  const getStaffDetails = async () => {
    setLoading(true);
    try {
      const apiRes = await henceforthApi.SuperAdmin.getStaffDetails(
        String(params?._id)
      );
      const modulePermissions = apiRes?.data?.module_permission || [];
      setUpdatedUserTabs(modulePermissions);
      form.reset({
        name: apiRes?.data?.name,
        email: apiRes?.data?.email,
        phoneNumber: apiRes?.data?.phone_no,
        countryCode: apiRes?.data?.country_code,
        module_permission: modulePermissions,
      });

      setImageState({
        preview: henceforthApi?.FILES.imageOriginal(
          apiRes?.data?.profile_pic,
          ""
        ),
        uploading: false,
        uploadedFileName: apiRes?.data?.profile_pic,
      });
    } catch (error) {
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (data: ProfileFormValues) => {
    try {
      ...
      const payload = {
        ...
        module_permission: updatedUserTabs,
      };

      if (imageState?.uploadedFileName) {
                            ...
                            <FormControl>
                              <MultiSelect
                                ...
                                onValueChange={(value) => {
                                  ...
                                  setUpdatedUserTabs(value);
                                }}
                                defaultValue={updatedUserTabs}
