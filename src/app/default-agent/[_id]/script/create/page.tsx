...
function WebPageView() {
  ...
  useEffect(() => {
    // setValue("logo", userInfo?.workspace?.image);
    setValue("description", userInfo?.workspace?.description);
    setValue("title", userInfo?.workspace?.name);
  }, []);
  ...
  const onSubmit = async (data: WebpageSettings) => {
    ...
    try {
      ...
      router.replace(
        `/default-agent/${agentId}/view?callPage=1&chatPage=1&voiceChatPage=1`
      );
    } catch (error) {
      Toast.error(error);
    } finally {
