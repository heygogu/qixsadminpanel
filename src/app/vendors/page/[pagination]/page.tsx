        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-2 w-56 lg:w-32">
                <Avatar className="h-10 w-10 shadow-md border-2 border-white">
                    <AvatarImage className='object-cover' src={henceforthApi.FILES.imageOriginal(row.original?.profile_pic || '')} alt={row.original.name} />
    ...
    // },
    {
        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-2">
                ...
                <span className='capitalize'>{row.original.type.slice(0, 1) + row.original.type.slice(1).toLowerCase()}</span>
            </div>
        ),
    },
    {
        ...
        cell: ({ row }) => (
            <Badge
                ...
                className={`shadow-md ${row.original.status === "ACTIVE"
                    ? "bg-green-100 text-green-800"
                    : "bg-yellow-100 text-yellow-800"
                    }`}
            >
                {row.original.status === "ACTIVE" ? "Active" : row.original.status === "INACTIVE" ? "Inactive" : "Unknown"}
            </Badge>
        ),
    },
    {
...
const VendorListing = () => {
    ...
    const [vendorListing, setVendorListing] = React.useState<{ data: Vendor[], count: number }>({
        data: [],
        count: 0,
    });
    ...
    const initData = async () => {
        ...
        try {
            const apiRes = await henceforthApi.SuperAdmin.vendorListing(urlSearchParams)
            setVendorListing({ ...vendorListing, data: apiRes?.data?.data, count: apiRes?.data?.count })
        } catch (error) {
            console.log(error)
        } finally {
            setIsLoading(false)
    ...
    const handlePageChange = (page: number) => {
        ...
        router.replace(
            `/vendors/page/${page}?${query.toString()}`,
            { scroll: false }
        );
    };
